package pahakia.samples.spring.gplus.jpa.controller;
...
public class AppController {
    ...
    public void gplusPost(HttpSession session, HttpServletResponse response, @RequestParam("state") String state,
        ...
        user.setFederatedIdentity(gplusId);
        user.setEmail("hello@world.com");
        user.setRole("User");
        try {
            userService.save(user);
        } catch (DataIntegrityViolationException ex) {
            PhkUser user2 = userService.findByFederatedId(gplusId);
            System.out.println("user already exists: " + user2);
        } catch (Throwable ex) {
            ex.printStackTrace();
        }
    ...
    public ModelAndView getUser(HttpSession session) {
        PhkUser user = userService.findByFederatedId((String) session.getAttribute("token"));
        ModelAndView model = new ModelAndView();
